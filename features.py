...
class FeatureExtractor():
    ...
    def _compute_formant_features(self, window):
        ...
        """
        (frequencies, bandwiths) = self._compute_formants(window)
        (dist, _bins) = np.histogram(frequencies, 15, (0, 5500))
        return dist # returns dummy value; replace this with the features you extract
    ...
    def _compute_delta_coefficients(self, window, N=2):
        ...
        for t in range(N, n_frames - N):
            numerator = 0
            for n in range(1, N):
                numerator += n * (mfccs[t + n] - mfccs[t - n])
